...
# Description:
#
#   Backlog section generator for the Year Planner. Generates multi-page backlog tables for unscheduled tasks
#   and ideas.
#-----------------------------------------------------------------------------------------------------------------------
...
#
#   Creates a configurable number of backlog pages. Each page has a table with a title row and blank content rows
#   for user entries. Double-sided (tables on both recto and verso).
#
# Arguments:
#
...
def generate_backlog ( document: Document, config: Config ) -> None:
    ...
    row_count      = backlog_config.get ( 'row_count', 24 )
...
#-----------------------------------------------------------------------------------------------------------------------
# Function: _create_backlog_table
#
# Description:
#
...
def _create_backlog_table ( document: Document, config: Config,
    ...
    # Backlog table has no header row, only a title row.
    ...
    _add_cell_text ( title_cell, "Backlog", size = title_font_size, bold = True, color = title_font_color, align = None )
